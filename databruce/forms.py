...
class CustomDateField(forms.DateField):
  ...
  def to_python(self, value):
    ...
    try:
      return datetime.datetime.strptime(value.strip(), "%Y-%m").date().replace(day=1)
    except ValueError:
      pass
...
class AdvancedEventSearch(forms.Form):
  ...
  type = CustomMultipleChoiceField(
    label="Event Type",
    lookup_path="event_type__type_id",
    required=False,
  ...
  continent = CustomCharField(
    label="Continent",
    lookup_path="venue__city__country__continent_id",
    required=False,
    widget=forms.Select(
      attrs={
        "class": "form-select form-select-sm select2",
